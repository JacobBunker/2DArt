...
def xfun(x,y,t,z):
    newRadius = (360*4)*((1/4)**z)
    return x+(newRadius*np.cos(np.radians(t/4)))
    
def yfun(x,y,t,z):
    newRadius = (360*4)*((1/4)**z)
    return y+(newRadius*np.sin(np.radians(t/4)))       
...
def drawCircle(x,y,z,level):
    ...
    if(level < 2):
        ...
        recursive(x-(nextRadius/2),y-(nextRadius/2),level+1)
...
def recursive(x,y,level):
    ...
    drawCircle(x+(adj),y,z,level)
    drawCircle(x+(adj*2),y,z,level)
    
    drawCircle(x,y+adj,z,level)
    drawCircle(x+(adj),y+adj,z,level)
    drawCircle(x+(adj*2),y+adj,z,level)
    
    drawCircle(x,y+(adj*2),z,level)
    drawCircle(x+adj,y+(adj*2),z,level)
    drawCircle(x+(adj*2),y+(adj*2),z,level)
...
if(True):
    level = 1
    rad = (360*4)*((1/4)**level) 
    recursive(x-(rad/2),y-(rad/2),level) 
