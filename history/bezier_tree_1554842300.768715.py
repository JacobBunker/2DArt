...
def getCurves(start, curves, distance):
    ...
    while(i < points.shape[0]):
        if(i == points.shape[0] - 1 or i%3 != 0):
            points[i] = getNext(points[i - 1],distance,(i+zSkew))
            i += 1
        else: #at the fourth point
            points[i+1] = getNext(points[i - 1],distance*2,(i+zSkew))
...
def drawCircle(x,y,radius,color="red"):
...
def drawLines(start,curves,distance,level):
    points = getCurves(start,curves,distance)
    ...
    while(z < points.shape[0]-1):
        ...
        if(level < 2):
            drawCircle(points[z+3,0],points[z+3,1],3,"red")
            if(level%2 == 0):
                drawLines(points[z+3],int(curves/4),distance*3,level+1)
            else:
                drawLines(points[z+3],int(curves),distance/4,level+1)
        else:
            drawCircle(points[z+3,0],points[z+3,1],2,"green")
            drawCircle(points[z+3,0],points[z+3,1],4,"green")
            #drawCircle(points[z+3,0],points[z+3,1],6,"green")
            drawCircle(points[z+3,0],points[z+3,1],8,"green")
            drawCircle(points[z+3,0],points[z+3,1],10,"green")
        z += 3
        

start = np.asarray([picSizeX/2, picSizeY/2])        
drawLines(start,24,10,0)
