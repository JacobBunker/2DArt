...
def getNext(p,d):
    return np.asarray([p[0] + (np.random.random()-0.5)*d, p[1] + (np.random.random()-0.5)*d])
...
d = 50
points[0] = start
points[1] = getNext(points[0],d)
points[2] = getNext(points[1],d)
points[3] = getNext(points[2],d)
